	...
	af[#af+1] = Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			OptionRowChangedMessageCommand=function(self)
				...
				self:stoptweening():smooth(0.2):diffusealpha((row_num >= 2 and row_num <= 8) and 1 or 0) -- Only show the notefield when the player is highlighting options that update it (the engine options can't properly do this)
			end,
