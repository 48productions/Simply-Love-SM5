...
for key, item in pairs(PaneItems) do
	...
	af[#af+1] = Def.ActorFrame{
		...
		LoadFont("Common Normal")..{
			...
			SetCommand=function(self)
				local SongOrCourse = (GAMESTATE:IsCourseMode() and GAMESTATE:GetCurrentCourse()) or GAMESTATE:GetCurrentSong()
				if not SongOrCourse then self:settext("? " .. key); return end
				...
				if steps then
					...
					self:settext( val >= 0 and val .. " " .. key or "? " .. key )
				else
					self:settext( "" )
				end
			end
...
for key, item in pairs(HighScoreItems) do
	af[#af+1] = Def.ActorFrame{
		...
		OnCommand=function(self)
			...
			-- Rival high scores should only be shown if the player has a profile (the rival label is also used for GrooveStats status messages, so show it even if GS isn't connected/etc)
		...
		LoadFont("Common Normal")..{
			...
			Name="HighScoreLabel",
			OnCommand=function(self)
				self:xy(item.labelx, item.y):zoom(textZoom * 0.7):diffuse(color_slate2):horizalign(center):maxwidth(90)
